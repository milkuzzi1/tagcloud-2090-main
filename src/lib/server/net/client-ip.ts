...
function getTrustedCidrs(): Cidr[] {
 ...
 const list = envValue?.trim() ? envValue.split(',') : DEFAULT_TRUSTED_CIDRS;
 const parsed: Cidr[] = [];
...
export function parseClientIp(
 ...
 // Все hop'ы доверенные (странно, но возможно при многослойной инфре) —
 // первый hop в цепочке считаем клиентом.
