...
// maxPayload: входящие сообщения от клиента — это только крошечные {type:'ping'}.
// 16 КБ с запасом; больше — рвём (защита от раздувания памяти на parse).
...
wss.on(
  ...
  async (
    ...
    ctx: { code: string; questionIds: string[]; maxWords: number; ip: string }
  ) => {
    const room = getRoom(ctx.code, ctx.questionIds, ctx.maxWords);
    const ok = await addSubscriber(room, ws, ctx.ip);
    if (!ok) {
      // Превышены лимиты комнаты/IP — мягко закрываем.
      ws.close(1013, 'busy');
      return;
    }
...
const wssUser = new WebSocketServer({ noServer: true, maxPayload: 16 * 1024 });
wssUser.on('connection', (ws: WebSocket, _req: IncomingMessage, ctx: { userId: string }) => {
  if (!addUserSubscriber(ctx.userId, ws)) {
...
export async function handleUpgrade(
  ...
  // Креаторский режим: доступ либо по ?t=<creatorToken> (бэккомпат), либо по
  // session-cookie владельца. Если нет ни токена, ни валидной сессии — рвём
  // сразу, не ходя в БД. (Токен из URL депрекейтим — он течёт в логи/Referer.)
  ...
  // Креаторский режим — доступ по токену (constant-time) ИЛИ по сессии владельца.
  if (!isPublic) {
    const tokenOk = token != null && constantTimeEqual(survey.creatorToken, token);
    const sessionOk = sessionUserId != null && survey.userId === sessionUserId;
