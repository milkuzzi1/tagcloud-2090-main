...
export const POST: RequestHandler = async ({ request, url, locals }) => {
  ...
  if (result.sent) {
    // resolvePublicBaseUrl fail-closed в проде: НЕ доверяем Host-заголовку
    // (url.origin) для построения ссылки сброса — иначе host-header injection
    // позволил бы отправить жертве ссылку на чужой домен.
    const baseUrl = resolvePublicBaseUrl(url.origin);
    const resetUrl = `${baseUrl}/reset-password?t=${result.token.token}`;
    const organizationName =
      env.APP_NAME || '\u041e\u0431\u043b\u0430\u043a\u043e \u0442\u0435\u0433\u043e\u0432 2090';
    try {
