...
> Можно не выполнять команды руками: в `deploy/dynamic-ip-setup.sh` лежит
> интерактивный мастер, который проводит по тем же шагам. Он показывает
> каждую команду до выполнения, спрашивает «выполнить / пропустить /
> своя команда / выйти», ведёт прогресс-бар и умеет возвращаться назад.
> Перед боевым запуском прогоните его в `DRY_RUN=1` — посмотреть, что
> именно будет сделано:
>
> ```bash
> DRY_RUN=1 bash deploy/dynamic-ip-setup.sh
> # Затем, когда готовы — запуск без DRY_RUN:
...
> исходящим коннектом к `smtp-pulse.com:587`. Раздел
...
Поэтому tagcloud по умолчанию ходит через **SendPulse SMTP** —
исходящим TLS-коннектом на `smtp-pulse.com:587`, авторизуясь
...
`smtp-pulse.com`, а не ваш домашний IP.

Краткий путь (полная инструкция — `deploy/mail-server.md`):

1. Зарегистрируйтесь на https://sendpulse.com и включите SMTP в кабинете.
2. Добавьте и верифицируйте домен отправителя (SPF/DKIM).
...
`2090.fun` в дашборде SendPulse и пропишите DNS:

| Тип | Имя | Значение |
|---|---|---|
| TXT | `@` (SPF) | `v=spf1 include:sendpulse.com ~all` |
| CNAME / TXT | (по инструкции SendPulse) | DKIM-запись из дашборда SendPulse |
| TXT | `_dmarc` | `v=DMARC1; p=quarantine; rua=mailto:postmaster@2090.fun; adkim=s; aspf=s` |

Лимиты SendPulse зависят от тарифа. Если объёмов перестанет
