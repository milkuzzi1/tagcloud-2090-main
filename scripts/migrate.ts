 ...
 *
 * ⚠️ Известная коллизия порядкового номера: существуют ДВА файла с префиксом
 * `0003` — `0003_invite_note.sql` и `0003_remove_organizations.sql`. Сортировка
 * здесь чисто лексикографическая (`Array.sort()` по имени файла), поэтому
 * порядок детерминирован: `0003_invite_note.sql` применяется ПЕРЕД
 * `0003_remove_organizations.sql` ('i' < 'r'). Эти миграции уже применены в
 * проде в этом порядке, поэтому переименовывать их НЕЛЬЗЯ — раннер трекает их
 * по имени файла в `__migrations`, и переименование выглядело бы как новая,
 * непримененная миграция (и/или потеря истории). Любые НОВЫЕ миграции должны
 * использовать уникальный, монотонно растущий префикс (следующий — `0008`).
 */
